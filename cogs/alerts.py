...
from database.db import (
    ...
    count_user_alerts, count_vol_configs, get_scheduler_channel,
)
...
class Alerts(commands.Cog):
    ...
    async def alert(
        ...
        symbol: app_commands.Range[str, 1, 10],
        ...
        count = await count_user_alerts(str(interaction.user.id), str(interaction.guild_id))
        if count >= MAX_ALERTS_PER_USER:
            await interaction.response.send_message(
                f"You already have {MAX_ALERTS_PER_USER} active alerts. Remove some with `/alert_remove` first.",
                ephemeral=True,
            )
            return
    ...
    async def volconfig(self, interaction: discord.Interaction, symbol: app_commands.Range[str, 1, 10], threshold: float = DEFAULT_VOL_THRESHOLD):
        symbol = symbol.upper()

        vol_count = await count_vol_configs(str(interaction.guild_id))
        if vol_count >= MAX_VOL_CONFIGS:
            await interaction.response.send_message(
                f"Vol alert limit reached ({MAX_VOL_CONFIGS} max).", ephemeral=True
            )
            return
    ...
    async def _check_price_alerts(self):
        ...
        for alert in active_alerts:
            ...
            if triggered:
                ...
                channel_id = await get_scheduler_channel(alert["guild_id"])
                if not channel_id:
                    continue
                ...
                if not channel:
                    continue
                await channel.send(
                    f"🚨 <@{alert['user_id']}> **{alert['symbol']}** has {reason}! "
